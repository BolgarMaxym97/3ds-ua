...
Патч перейменовує монтування на `rex:` — ті самі чотири байти, на місці, у трьох копіях рядка. Далі 28 слів, які будували сталий шлях `man:/Manual.bcma`, замінено на побудову шляху з ID документованого титулу через `snprintf` самого титулу (`0x157EDC`, формат ліг у 20 мертвих байтів колишнього шляху):

```
rex:/%08x%08x.bcma  ->  /luma/titles/0004003000009B02/romfs/<tid>.bcma
```

Відкат безкоштовний: payload Luma, не відкривши файл на SD, повторює виклик з оригінальними аргументами — тобто титул без перекладу показує рідний посібник консолі. Деталі — `manual_path` у `tools/luma_hook.py`.
